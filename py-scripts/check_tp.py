...
class Throughput(Realm):
    ...
    def phantom_check(self):
        ...
        configure_list=[]
        if len(self.device_list)==0 and self.config == False and self.group_name==None:
        ...
        print("realclientlist",self.real_client_list)
    ...
    def monitor(self, iteration, individual_df, device_names, incremental_capacity_list, overall_start_time, overall_end_time):
        ...
        while datetime.now() < end_time:
            ...
            if self.dowebgui:
                ...
                if self.dowebgui == True and self.group_name!=None:
    ...
    def generate_report(self, iterations_before_test_stopped_by_user, incremental_capacity_list, data=None, data1=None, report_path='', result_dir_name='Throughput_Test_report',
        ...
        if self.do_interopability == False:
            ...
            for i in range(len(iterations_before_test_stopped_by_user)):
                ...
                test_input_list=[]
                pass_fail_list=[]
                if(self.expected_passfail_value!=None or self.device_csv_name!=None):
                    if(self.expected_passfail_value=='' or self.expected_passfail_value==None): 
                        res_list=[]
                ...
                print('UPLOAD',upload_drop)
                print('download',download_drop)
                if self.group_name!=None:
                    for key,val in self.groupdevmap.items():
                        if(self.expected_passfail_value!=None or self.device_csv_name!=None):
                            dataframe=self.generate_dataframe(val,device_type[0:int(incremental_capacity_list[i])],devices_on_running[0:int(incremental_capacity_list[i])],self.ssid_list[0:int(incremental_capacity_list[i])],self.mac_id_list[0:int(incremental_capacity_list[i])],self.channel_list[0:int(incremental_capacity_list[i])], self.mode_list[0:int(incremental_capacity_list[i])],direction_in_table[0:int(incremental_capacity_list[i])], download_list[0:int(incremental_capacity_list[i])],[str(n)+" Mbps" for n in download_data[0:int(incremental_capacity_list[i])]],upload_list[0:int(incremental_capacity_list[i])],[str(n)+" Mbps" for n in upload_data[0:int(incremental_capacity_list[i])]],['' if n == 0 else '-' + str(n) + " dbm" for n in rssi_data[0:int(incremental_capacity_list[i])]],test_input_list,self.link_speed_list[0:int(incremental_capacity_list[i])],[str(n) for n in packet_size_in_table[0:int(incremental_capacity_list[i])]],pass_fail_list,upload_drop,download_drop)
                        else:
                            dataframe=self.generate_dataframe(val,device_type[0:int(incremental_capacity_list[i])],devices_on_running[0:int(incremental_capacity_list[i])],self.ssid_list[0:int(incremental_capacity_list[i])],self.mac_id_list[0:int(incremental_capacity_list[i])],self.channel_list[0:int(incremental_capacity_list[i])], self.mode_list[0:int(incremental_capacity_list[i])],direction_in_table[0:int(incremental_capacity_list[i])], download_list[0:int(incremental_capacity_list[i])],[str(n)+" Mbps" for n in download_data[0:int(incremental_capacity_list[i])]],upload_list[0:int(incremental_capacity_list[i])],[str(n)+" Mbps" for n in upload_data[0:int(incremental_capacity_list[i])]],['' if n == 0 else '-' + str(n) + " dbm" for n in rssi_data[0:int(incremental_capacity_list[i])]],[],self.link_speed_list[0:int(incremental_capacity_list[i])],[str(n) for n in packet_size_in_table[0:int(incremental_capacity_list[i])]],[],upload_drop,download_drop)                            
                    ...
                    if(self.expected_passfail_value!=None or self.device_csv_name!=None):
                        bk_dataframe[" Expected "+direction+" rate "] = [str(n) + " Mbps" for n in test_input_list]
            ...
            for i in range(len(iterations_before_test_stopped_by_user)):
                ...
                if self.group_name!=None:
                    ...
                    for key,val in self.groupdevmap.items():
                        if(self.expected_passfail_value!=None or self.device_csv_name!=None):
                            dataframe=self.generate_dataframe(val,device_type[int(incremental_capacity_list[i])-1],[devices_on_running[-1]],self.ssid_list[int(incremental_capacity_list[i])-1],self.mac_id_list[int(incremental_capacity_list[i])-1],self.channel_list[int(incremental_capacity_list[i])-1], self.mode_list[int(incremental_capacity_list[i])-1],direction_in_table[-1], download_list[-1],[str(download_data[-1])+" Mbps"],upload_list[-1],[str(upload_data[-1])+" Mbps" ],['' if rssi_data[-1] == 0 else '-'+str(rssi_data[-1])+ " dbm"],test_input_list,self.link_speed_list[int(incremental_capacity_list[i])-1],[],pass_fail_list,upload_drop,download_drop)
                        else:
                            dataframe=self.generate_dataframe(val,[device_type[int(incremental_capacity_list[i])-1]],[devices_on_running[-1]],self.ssid_list[int(incremental_capacity_list[i])-1],self.mac_id_list[int(incremental_capacity_list[i])-1],self.channel_list[int(incremental_capacity_list[i])-1], self.mode_list[int(incremental_capacity_list[i])-1],direction_in_table[-1], download_list[-1],[str(download_data[-1])+" Mbps"],upload_list[-1],[str(upload_data[-1])+" Mbps" ],['' if rssi_data[-1] == 0 else '-'+str(rssi_data[-1])+ " dbm"],[],self.link_speed_list[int(incremental_capacity_list[i])-1],[],[],upload_drop,download_drop)
                            # dataframe=self.generate_dataframe(val,device_type[0:int(incremental_capacity_list[i])],devices_on_running[0:int(incremental_capacity_list[i])],self.ssid_list[0:int(incremental_capacity_list[i])],self.mac_id_list[0:int(incremental_capacity_list[i])],self.channel_list[0:int(incremental_capacity_list[i])], self.mode_list[0:int(incremental_capacity_list[i])],direction_in_table[0:int(incremental_capacity_list[i])], download_list[0:int(incremental_capacity_list[i])],[str(n)+" Mbps" for n in download_data[0:int(incremental_capacity_list[i])]],upload_list[0:int(incremental_capacity_list[i])],[str(n)+" Mbps" for n in upload_data[0:int(incremental_capacity_list[i])]],['' if n == 0 else '-' + str(n) + " dbm" for n in rssi_data[0:int(incremental_capacity_list[i])]],[],self.link_speed_list[0:int(incremental_capacity_list[i])],[str(n) for n in packet_size_in_table[0:int(incremental_capacity_list[i])]],[],upload_drop,download_drop)                            

                            print('data_frame1',dataframe)
                            # dataframe=self.generate_dataframe(val,device_type[int(incremental_capacity_list[i])-1],devices_on_running[-1],self.ssid_list[int(incremental_capacity_list[i])-1],self.mac_id_list[int(incremental_capacity_list[i])-1],self.channel_list[int(incremental_capacity_list[i])-1], self.mode_list[int(incremental_capacity_list[i])-1],direction_in_table[-1], download_list[-1],[str(download_data[-1])+" Mbps"],upload_list[-1],[str(upload_data[-1])+" Mbps" ],['' if rssi_data[-1] == 0 else '-'+str(rssi_data[-1])+ " dbm"],[],self.link_speed_list[int(incremental_capacity_list[i])-1],[],[],upload_drop,download_drop)
                            # print('data_frame2',dataframe)
                        if dataframe!=0:
                            report.set_obj_html("","Group: {}".format(key))
                            report.build_objective()
                            dataframe1 = pd.DataFrame(dataframe)
    ...
    def generate_dataframe(self,groupdevlist,typeofdevice,devusername,devssid,devmac,devchannel,devmode,devdirection,devofdownload,devobsdownload,devoffupload,devobsupload,devRSSI,devExpected,devlinkspeed,devpacketsize,devstatus,upload_drop,download_drop):
            print('FUPLOAD',upload_drop)
            print('Fdownload',download_drop)
            device_type = []
            username= []
            ssid = []
            mac = []
            ...
            avg_dndrop=[]
            # dir=dir
            print('DEV USER NAME',devusername)
            print('groupdev',groupdevlist)
            print('typeof[device]' , typeofdevice)
            interop_tab_data = self.json_get('/adb/')["devices"]
...
''')
    ...
    if(len(selected_groups)!=len(selected_profiles)):
        ...
        exit(0)
    elif(args.config and (args.ssid==None or (args.passwd==None and args.security==None) or(args.passwd==None and args.security.lower()!='open') )):
        print("Please provide ssid password and security for configuring devices")
        exit(0)
