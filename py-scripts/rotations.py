...
class Throughput(Realm):
    ...
    def monitor(self, iteration, individual_df, device_names, incremental_capacity_list, overall_start_time, overall_end_time,current_coordinate,rotation_list=None,target_x=None, target_y= None,angle_list= None,cxlist=None):
        ...
        for j in  range(len(rotation_list)):
            # pause_angle = self.wait_for_battery()
            ...
            if self.rotation_enabled:
                ...
                #     if abs(angle - theta) <= 0.15:
                #         break
                rotation =self.rotate_angle(target_x,target_y,angle)
                if not rotation:
                    exit_from_monitor=True
            ...
            while datetime.now() < end_time:
                ...
                if (datetime.now() - monitor_charge_time).total_seconds() >= 300:
                    ...
                    pause=self.wait_for_battery()
                    print("pauseee",pause,"rotationnenabled",self.rotation_enabled)
                    if pause:
                        self.stop()
                        reached = self.move_to_coordinate(current_coordinate)
                        if not reached:
                            test_stopped_by_user=True
                            break
                        if self.rotation_enabled:
                            print("enteredrotation")
                            rotation_moni =self.rotate_angle(target_x,target_y,angle)
                        ...
                        monitor_charge_time = datetime.now()
                        previous_time=monitor_charge_time
                        print("previoustimeee",monitor_charge_time)
                if self.dowebgui:
        ...
        if args.manual:
                    ...
                    if (robot_status =='Stopped'):              
                        ...
                        if current_coordinate == user_input:
                            ...
                            for i in range(len(to_run_cxs)):
                                ...
                                all_dataframes, test_stopped_by_user = throughput.monitor(i, individual_df, device_names, incremental_capacity_list, overall_start_time, overall_end_time,current_coordinate,to_run_cxs[i])
            ...
            for wp in data.get("waypoints", []):
                ...
                if wp.get("type") == "charge":
                    throughput.charge_point_name = wp["name"]
            for coord in coord_list:
                # pause_coord=throughput.wait_for_battery()
                if test_stopped_by_user:
                    break
